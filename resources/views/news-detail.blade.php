...
<body class="bg-gray-100 font-sans">
    ...
    <main class="container mx-auto max-w-5xl px-4 py-8">
        <div class="bg-white rounded-3xl shadow-xl p-6 md:p-12">
            <a href="{{ route('news') }}" class="inline-flex items-center text-orange-500 hover:text-orange-600 transition-colors duration-300 mb-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center text-sm text-gray-500 mb-6">
                <span class="flex items-center"><i class="fas fa-clock mr-2 text-orange-400"></i> {{ \Carbon\Carbon::parse($news->created_at)->format('d F Y H:i น.') }}</span>
                    ...
                    @php
                        ...
                        if ($diffInDays == 0) {
                            ...
                            if ($diffInHours == 0) {
                                ...
                                $timeAgo = $diffInMinutes > 0 ? "{$diffInMinutes} นาทีที่แล้ว" : "ไม่กี่วินาทีที่แล้ว";
                            } else {
                ...
                <a href="{{ $news->source_url }}" target="_blank" class="ml-auto text-orange-600 hover:underline mt-2 md:mt-0">
                    <span class="hidden md:inline-block">ที่มา:</span> ครูบ้านนอก
                </a>
            </div>

            <img src="{{ $news->image_url }}" alt="{{ $news->title }}" class="w-full h-auto rounded-2xl shadow-lg mb-8">
            ...
            <div class="text-gray-700 leading-relaxed text-lg space-y-6 text-justify">
                ...
                @foreach ($paragraphs as $index => $paragraph)
                    @php
                        ...
                        $formattedParagraph = preg_replace('/(https?:\/\/[^\s]+)/', '<a href="$1" class="text-orange-500 hover:underline" target="_blank">$1</a>', $formattedParagraph);
